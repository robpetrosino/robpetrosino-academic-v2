...
ggplot(data = spiderData)+ # the dataset it needs to retrieve the data from. This will just draw the frame of plot. You then add a `+` for further instructions to give for plotting
  geom_histogram(aes(x = Anxiety)) # after the ggplot() function you then specify what kind of plot you want. All kinds of plots have the same form: geom_*(). In this case we want a histogram, so the right function would be: geom_histogram. The one argument you need to finally plot the historam is the aes() function, which stands for "aesthetics" and basically takes the variables you want to plot in the x- and y-axes. Given that this is an histogram, the y-axis is predefined as frequency/count; but the x-axis must be associated to the right variable/column of the dataset define in the line above. What is that variable?
...
distrPlot <- ggplot(data = spiderData)+ 
  geom_histogram(aes(x = Anxiety), color='yellow', fill='black') #add the name of the color you want instead of the ____. the `color` argument will change the border color; the `fill` argument will change the filling color of the bars!
distrPlot # Remember that, if you assign your plot a name, you will have to call the plot to make it appear in the Plots tab in the lower right panel
...
spiderSummary <- spiderData %>% # first, let's use our %>% (pipe) operator. Remember, think about it as turbine
  group_by(Group) %>% # the group_by() function will split the original dataset in two depending on the values of the variable/column we put inside of it. Notice that %>% will have to be typed in at every step
  summarise(meanAnxiety = mean(Anxiety), 
            sdAnxiety = sd(Anxiety),
            obs = n(), # this function (notice that it has the parentheses, but no arguments!) will return the number of observations/rows for each value of the variable Group
            seAnxiety= sdAnxiety/sqrt(obs)) # the summarise() function will make up a *brand-new* dataset from the original dataset and takes as many variables as we want.
...
plotMeans <- ggplot(data = spiderSummary)+ # the dataframe first -- which one are we using?
  geom_col(aes(x = Group, y = meanAnxiety))+ # which geometry should we use? Let's use the helper to find out!
  geom_errorbar(aes(x= Group, ymax = meanAnxiety + seAnxiety, ymin = meanAnxiety - seAnxiety), width = .2)+ # we also want to put standard error bars -- let's see if the helper could lead us in the right direction
  labs(title = "Arachnophobia experiment", x = "group", y = "Anxiety level") # finally, we would also like to add a title for the plot and new labels for the axes -- check the helper for further information about the labs() function
...
is.factor(spiderSummary$Group)

# Ha! It's not a factor. We have to fix that. How do we do it? We use the as.factor() function

spiderSummary$Group <- as.factor(spiderSummary$Group)

is.factor(spiderSummary$Group) # check again if now Group is a factor.
...
levels(spiderSummary$Group) # type in the predictor variable of the dataset
...
spiderSummary$Group <- fct_recode(spiderSummary$Group, Picture = "picture", `Real Spider` = "real-spider") # the ticks `` are used to help R consider spaced-out strings as a whole
...
plotMeansTwo <- ggplot(data = spiderSummary)+ # the dataframe first -- which one are we using?
  geom_col(aes(x = Group, y = meanAnxiety))+ # which geometry should we use? Let's use the helper to find out!
  geom_errorbar(aes(x= Group, ymax = meanAnxiety + seAnxiety, ymin = meanAnxiety - seAnxiety), width = .2)+ # we also want to put standard error bars -- let's see if the helper could lead us in the right direction
  labs(title = "Arachnophobia experiment", x = "group", y = "Anxiety level") # finally, we would also like to add a title for the plot and new labels for the axes -- check the helper for further information about the labs() function
...
qnorm(p=(0.32/2), lower.tail = FALSE)
