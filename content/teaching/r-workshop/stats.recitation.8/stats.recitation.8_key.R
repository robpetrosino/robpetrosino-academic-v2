...
wiiSummary <- wii %>% 
  ...
  summarise(meanInjury=mean(injury), minInjury=min(injury), maxInjury=max(injury),
            sdInjury=sd(injury), nObs=n(), seInjury=sdInjury/sqrt(nObs))  

## why don't we plot this?

interactionPlot <- ggplot(wiiSummary, aes(x=athlete, y=meanInjury, # let's put the athlete factor on the x-axis
                                          ...
                                          color=stretch, # `color` wants the *variable name* which we want the contour of the geometries below to change by
                                          fill=stretch,  # `fill` wants the *variable name* which we want the filling of the geometries below to change by
                                          group=stretch)) + # `group` wants the *variable name* which we want to group the dataset by
  geom_point() + geom_line() # plot overlaying point and line geometries
...
model <- aov(injury ~ athlete*stretch, data=wii) # we should name it so to call it
model # like this
